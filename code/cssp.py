...
def main(n,
    ...
    for _ in range(n_external_iters):
        ...
        with prw.compute():  # Enter a compute block to allow clean uncomputation of the walk later
            ...
            # In Szegedy's algorithm over the complete graph (and Johnson graph variations without bounded spatial locality needed),
            # the Walk operator W = Ref_B * Ref_A collapses mathematically to Amplitude Amplification over the steady-state
            # generated by the Vertex Binary Encoding operators.
            # To fix the "asymmetric path garbage parameter" bug in bartschiE19's 'update' operator,
            # we perform the exact Reflection around the initialized Dicke State (which forms the exact graph spectrum nodes).
        ...
        # 4. Recompute VBE matrix and Dicke state (handled natively by prw.uncompute() which inverses the compute block!)
        prw.uncompute()
